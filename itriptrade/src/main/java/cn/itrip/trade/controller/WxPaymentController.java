...
public class WxPaymentController {
    ...
    public Dto createCode(@PathVariable String orderNo) {
        try {
            ...
            data.put("total_fee", order.getPayAmount().multiply(new BigDecimal(100)).toBigInteger().toString()); //订单的金额， 乘以100得到元为单位。
            data.put("spbill_create_ip", "123.12.12.123");
            data.put("nonce_str", WXPayUtil.generateNonceStr());//随机字符串
            //异步通知的URL地址。
            data.put("notify_url", "http://139.159.163.127/itriptrade/api/wx/notify");
            data.put("trade_type", "NATIVE");
            data.put("product_id", "12");
            data.put("appid", "wxab8acb865bb1637e");//appid暂时用的是测试用的appid
            data.put("mch_id", "11473623"); //商户号
